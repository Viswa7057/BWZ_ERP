...
if ($_POST['accept_neworder']) {
  ...
  $stmt_update = "UPDATE order_details SET ceo_status='Accepted',status='OPS' WHERE id='$id'";

  $rslt_update = mysqli_query($conn, $stmt_update);

  $stmt_acc = "SELECT email,user_name from users where user_level='Accounts';";
  $rslt_acc = mysqli_query($conn, $stmt_acc);
  $row_acc = mysqli_fetch_row($rslt_acc);
  $acc_email = $row_acc[0];
  $acc_user = $row_acc[1];
  ...
  $mail->isHTML(true);
  $mail->Subject = "New Order Info - Request For Invoice";
  $mail->Body = "<p>Dear $acc_user, </p> <h3>New Order Info has been added for Invoice, kindly check the dash board for more details.<br></h3>
  ...
  $mail->Body = "<p>Dear $ops_user, </p> <h3>New Order Info has been added, kindly check the dash board for more details.<br></h3>
                     Click the below link for login<br>
					 http://brandonwheelz.in/index.php<br>
                     <p></p>
                     <b></b>";
...
if ($_POST['reject_neworder']) {
  ...
  $stmt_update = "UPDATE order_details SET ceo_status='Rejected',comments='$txt_reason' WHERE id='$txt_id'";

  $rslt_update = mysqli_query($conn, $stmt_update);

  $stmt_rs = "SELECT sales_person from order_details where id='$txt_id';";
  $rslt_rs = mysqli_query($conn, $stmt_rs);
  while ($row = mysqli_fetch_assoc($rslt_rs)) {

    $loginUser = $row["sales_person"];

    $stmt_rs = "SELECT email from users where user_name='$loginUser';";
    $rslt_rs = mysqli_query($conn, $stmt_rs);
    $row_rs = mysqli_fetch_row($rslt_rs);
    $sales_email = $row_rs[0];
  }
  ...
  $mail->setFrom('erp@brandonwheelz.com', 'ERP');
  $adminEmail = "$sales_email"; // Replace with the desired email address
  $mail->addAddress($adminEmail);

  $mail->isHTML(true);
  $mail->Subject = "New Order Info";
  $mail->Body = "<p>Dear $loginUser, </p> <h3>Your new order has been rejected by CEO kindly check the dash board for more details<br></h3>
                    Click the below link for login<br>
					http://brandonwheelz.in/index.php<br>
                     <p></p>
                     <b></b>";
  ...
  <div class="wrapper">
    ...
    <div class="content-wrapper">
      ...
      <section class="content" style="background: linear-gradient(327deg, #ff0000a3, #FFFFFF), url('../images/bwz_erp_bg.jpg'); background-size: cover; background-position: center center;">
                  ...
                  <div class="card-body p-0">
                    <table class="table table-striped projects table-bordered" id="table_camp">
                      ...
                      <tbody>
                        ...
                        <?php
                       //$stmt_select = "SELECT * FROM order_details order by id desc;";
					  $stmt_select = "SELECT * FROM order_details WHERE HOD_Status = 'Accepted' ORDER BY id DESC";
                        ...
                        //   $Po_file = "../Sales/PO/" . $row["PO"];
                                 $Po_files = explode('|', $row["PO"]); // assuming "file_name" is the column storing PO file names
                          $check_file = "../Sales/checklist/" . $row["Checklist"];
                          $PI_file = "../Accounts/Invoice/" . $row["customer_invoice"];
                        ...
                        ?>
                          <tr>
                            ...
                            <td>
                              <?php if ($row["customer_invoice"] != "") { ?>
                                <button class="btn btn-success" onclick="viewPI('<?php echo $PI_file; ?>')"><i class="fa fa-eye"></i>Invoice</button>
                              <?php } else { ?>
                            ...
                            <td>
                              <!--<button class="btn btn-primary" onclick="viewInvoice('<?php echo $Po_file; ?>')"><i class="fa fa-eye"></i> PO</button><br><br>-->
                               <?php foreach ($Po_files as $index => $Po_file): ?>
								
								<button class="btn btn-primary" onclick="viewPO('<?php echo '../Sales/PO/' . htmlspecialchars($Po_file); ?>', <?php echo $index + 1; ?>)">
								<i class="fa fa-eye"></i> PO <?php echo $index + 1; ?>
									</button><br><br>
								<?php endforeach; ?>
							   <button class="btn btn-primary" onclick="viewCheck('<?php echo $check_file; ?>')"><i class="fa fa-eye"></i> Check</button><br><br>
                            ...
                            <td>
                              <?php if ($row["ceo_status"] == "Accepted" || $row["ceo_status"] == "Rejected" ) { ?>
                              <?php } else { ?>
                                ...
                                <br>
                                <button class="btn btn-danger btn-sm" style="width: 91px;" onclick="rejectInstallation('<?php echo $row['id']; ?>','<?php echo $row['company_name']; ?>')">Reject</button>
                              <?php
                              }
                              ?>
              ...
              <div class="modal-dialog" role="document">
                <div class="modal-content" style="width: 800px;margin-left: -112px;">
                  <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Vendor PO Request</h5>
                        ...
                        <tr>
                          ...
                          <th>
                            <div class="form-group" style="width:250px;">
                              ...
                              <select class="form-control" id="vendor_type" name="vendor_type">
                                ...
                                <option value="Vendor2">Fleet</option>
                                <option value="Vendor1">Mounting</option>
                                <option value="Vendor2">Auto Hood Manufaturer</option>
                                <option value="Vendor2">Auto Hood Installer</option>
                                <!-- Add more options as needed -->
                              </select>
                            </div>
                          </th>
                          <th>
                            <div class="form-group" style="width:250px;">
                              ...
                              <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter Campaing Code only" id="Campaing_Code" name="Campaing_Code" placeholder="Enter Campaing Code" required>
                            </div>
                          </th>
                        <tr>
                          <th>
                            <div class="form-group" style="width:250px;">
                              ...
                              <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter Invoice Value only" id="Invoice_Value" name="Invoice_Value" placeholder="Enter Company Name" required>
                            </div>
                          </th>
                          <th>
                            <div class="form-group" style="width:250px;">
                              <label for="UserID">Invoice (PDF Only) <span style="color:red;">*</span></label>
                              <input type="file" class="form-control-file" id="Invoice" name="Invoice" accept=".pdf" required>
                            </div>
                          </th>
                          <th>
                            <div class="form-group" style="width:250px;">
                              <label for="UserID">Car Images<span style="color:red;">*</span></label>
                              <input type="file" class="form-control-file" id="Car_Images" name="Car_Images[]" accept="image/*" multiple required>
                            </div>
                          </th>
                        </tr>
                  </div>
        ...
        <!-- The Modal -->
        <div class="modal fade" id="myModal_edit" style="opacity: 3;overflow-y:scroll;">
                    ...
                    <tr>
                      ...
                      <th>
                        <div class="form-group" style="width:250px;">
                          ...
                          <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter alphabet only" id="txt_Company_Name" name="txt_Company_Name" placeholder="Enter Company Name" readonly>
                        </div>
                      </th>
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="ProductID">Contact Person Name</label>

                          <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter alphabet only" id="txt_Contact_Person_Name" name="txt_Contact_Person_Name" placeholder="Enter Contact Person Name" readonly>
                        </div>
                      </th>
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="ProductID">Contact Person Mob No</label>

                          <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter alphabet only" id="txt_Contact_Person_Mob_No" name="txt_Contact_Person_Mob_No" placeholder="Enter Contact Person Mob No" readonly>
                        </div>
                      </th>
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="ProductID">Contact Person Email ID</label>
                          <input type="text" class="form-control" pattern="^[a-zA-Z\s]*$" title="Please enter alphabet only" id="txt_Email_ID" name="txt_Email_ID" placeholder="Enter Contact Person Email ID" readonly>
                        </div>
                      </th>
                      ...
                      <th>
                        <div class="form-group" style="width:250px;">
                          ...
                          <textarea class="form-control" pattern=".*" title="Please enter only alphabets and numbers" id="txt_Campaing_Details" name="txt_Campaing_Details" placeholder="Enter Campaing Details " readonly></textarea>
                        </div>
                      </th>
                    </tr>
                    ...
                    <tr>
                      <th>
                        <div class="form-group" style="width:250px;">
                          ...
                          <input type="date" class="form-control" id="txt_Start_date" name="txt_Start_date" readonly>
                        </div>
                      </th>
                      ...
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="UserID">Any Spl Request</label>
                          <textarea class="form-control" pattern=".*" title="Please enter only alphabets and numbers" id="txt_Spl_Request" name="txt_Spl_Request" placeholder="Enter Any Spl Request" readonly></textarea>
                        </div>
                      </th>
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="UserID">Media Type</label>
                          <select class="form-control select2" style="width: 100%;" id="txt_Media_Type" name="txt_Media_Type" readonly>
                      ...
                      <th>
                        ...
                        <div class="form-group" style="width:250px;">
                          ...
                          <input type="text" class="form-control" pattern="^[0-9\s]*$" title="Please enter Number only" id="txt_No_of_Vehicles" name="txt_No_of_Vehicles" placeholder="Enter Total No of Vehicles" readonly>
                        </div>
                      ...
                      <th>
                        <div class="form-group" style="width:250px;">
                          ...
                          <input type="text" class="form-control" pattern="^[0-9\s]*$" title="Please enter Number only" id="txt_GST" name="txt_GST" placeholder="Enter Total Value Without GST" readonly>
                        </div>
                      </th>
                    </tr>
                  ...
                  <table>
                    <tr>
                      ...
                      <th>
                        <div class="form-group" style="width:250px;">
                          <label for="UserID">Campaing Details <span style="color:red;"></span></label>
                          <textarea class="form-control" pattern=".*" title="Please enter only alphabets and numbers" id="Campaing_Details" name="Campaing_Details" placeholder="Enter Campaing Details "></textarea>
                        </div>
                      </th>
                    ...
                    <tr>
                      <th>
                        <div class="form-group">
                          <label for="IngroupName">PO<span style="color:red;">*</span></label>
                          <input type="file" class="form-control" id="PO" name="PO">
                        </div>
                      </th>
                      <th>
                        <div class="form-group">
                          <label for="IngroupName">Checklist<span style="color:red;">*</span></label>
        ...
        <div class="modal fade" id="myModal_delete" style="opacity: 3;top : 104px !important">
          <div class="modal-dialog">
            <div class="modal-content" style="top:40px;">
              ...
              <!-- Modal Header -->
              <div class="modal-header">
                <h4 class="modal-title">Reject New Installation</h4>
...
<script>
  ...
  <div id="popup_fuck" class="popup" style="display: none;margin-top: -1949px;margin-left: 985px;">
  ...
  <script>
    function fucking(PO, Check) {
      alert(PO);
      var popup = document.getElementById("popup_fuck");
