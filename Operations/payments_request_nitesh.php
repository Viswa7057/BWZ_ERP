...
if ($_POST['addInfo_list']) {
  ...
  if ((!in_array($fileExtension_VD, $allowedFileTypes_VD)) && (!in_array($fileExtension_DC, $allowedFileTypes_DC))) {

    echo '<script>alert("Sorry, Only PDF files are allowed.")</script>';
  } elseif ($fileType != 'application/zip' && $fileType != 'application/x-zip-compressed') {
    ...
    $stmt_update = "INSERT INTO payment_request(vendor_name, type_of_vendor, campaign_code, invoice, dc_report, vehicle_details, vehicle_images, Status) VALUES ('$vendor_Name','$vendor_type','$Campaing_Code','$invoice','$file_name_DC','$file_name_VD','$zipFilePath','Requested')";
    ...
    <aside class="main-sidebar" style=" background: linear-gradient(327deg, #ff0000a3, #9cb3d7  ); color: #FFFFFF;">
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          ...
          <li><a href="ops_report.php" class="menu-link"><i class="fa fa-align-center " aria-hidden="true"></i> Operations</a></li><br><br>
        </nav>
        <!-- /.sidebar-menu -->
      </div>
      <!-- /.sidebar -->
                  ...
                  <div class="card-body p-0">
                    <table class="table table-striped projects table-bordered" id="table_camp">
                      ...
                      <tbody>
                        ...
                        <?php
                        $stmt_select = "SELECT * from payment_request";
                        ...
                        ?>
                          <tr>
                            ...
                            <td>
                              <?php if ($row["ceo_status"] == "Accepted") { ?>
        ...
        <tr>
          <th>
            <div class="form-group" style="width:250px;">
              ...
              <select class="form-control select2" style="width: 100%;" name="vendor_Name" id="vendor_Name" onChange="fetchVendorDetails()" required>
                <option value="">Select Vendor Name</option>
                <?php
                  include('db_connect.php');
                  $stmt_select = "SELECT DISTINCT vendor_name,id FROM purchase WHERE status='Received'";
                  $rslt_rs = mysqli_query($conn, $stmt_select);
                  while ($row = mysqli_fetch_assoc($rslt_rs)) {
					// $id = $row['id'];
                ?>
                  <option value="<?php echo $row['vendor_name']; ?>" data-id="<?php echo $row['id']; ?>"><?php echo $row['vendor_name']; ?></option>
                <?php } ?>
              </select>
          ...
          <th>
            <div class="form-group" style="width:250px;">
              ...
              <input type="text" class="form-control" id="vendor_type" name="vendor_type" readonly>
            </div>
          </th>
          <th>
            <div class="form-group" style="width:250px;">
              <label for="Campaing_Code">Campaign Code<span style="color:red;">*</span></label>
              <input type="text" class="form-control" id="Campaing_Code" name="Campaing_Code" readonly required>
            </div>
          </th>
        </tr>
                <tr>
                  <th>
                    <div class="form-group" style="width:250px;">
                      ...
                      <input type="text" class="form-control" pattern="^[0-9\s]*$" title="Please enter only numbers" id="invoice" name="invoice" placeholder="Enter Invoice Value" required>
                    </div>
                  </th>
                  <th>
                    <div class="form-group">
                      <label for="IngroupName">Upload Vehical Details(Only PDF)<span style="color:red;">*</span></label>
                      <input type="file" class="form-control" accept=".pdf" id="vehicle_details" name="vehicle_details" required>
                    </div>
                  </th>
                  <th>
                    <div class="form-group">
                      <label for="IngroupName">Upload Vehical Images(Only ZIP)<span style="color:red;">*</span></label>
                ...
                <tr>
                  <th>
                    <div class="form-group">
                      <label for="IngroupName">Upload DC Report(Only PDF)<span style="color:red;">*</span></label>
                      <input type="file" class="form-control" accept=".pdf" id="dc_report" name="dc_report" required>
                    </div>
                  </th>
                </tr>
          </div>
     ...
     var selectedOption = $('#vendor_Name option:selected');
      ...
      if (vendorName !== '') {
        ...
        $.ajax({
          ...
          method: 'POST',
          data: { vendor_name: vendorName,id: id},
          dataType: 'json',
