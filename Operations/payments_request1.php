...
// Handle Vendor Payment Request Add
...
// Handle Vendor Payment Request Update
...
// Handle Internal Payment Request Add
if ($_POST['addInternal_payment']) {
  
  $employee_name = $_POST['internal_employee_name'];
  $total_amount = $_POST['internal_total_amount'];
  $comments = $_POST['internal_comments'];
  
  // Handle file upload
  $file_name_details = $_FILES['internal_upload_details']['name'];
  $upload_dir_details = 'Internal_Payment_Details/';
  
  // Create directory if it doesn't exist
  if (!file_exists($upload_dir_details)) {
    mkdir($upload_dir_details, 0777, true);
  }
  
  $file_path_details = $upload_dir_details . $file_name_details;
  
  // Validate PDF file
  $fileExtension = strtolower(pathinfo($file_name_details, PATHINFO_EXTENSION));
  
  if ($fileExtension == 'pdf' || $fileExtension == '') {
    move_uploaded_file($_FILES['internal_upload_details']['tmp_name'], $file_path_details);
    
    $stmt_insert = "INSERT INTO internal_payment_request(employee_name, upload_details, total_amount, comments, payment_type, status) 
                    VALUES ('$employee_name', '$file_name_details', '$total_amount', '$comments', 'Internal Payment', 'Requested')";
    
    $rslt_insert = mysqli_query($conn, $stmt_insert);
    
    if($rslt_insert) {
     // echo '<script>alert("Internal Payment Request added successfully!")</script>';
      ...
      $mail->isHTML(true);
      $mail->Subject = "New Internal Payment Request";
      $mail->Body = "<p>Dear $ops_user, </p> <h3>New Internal Payment Request has been added. Kindly check the dashboard for more details.<br></h3>
                       Click the below link for login<br>
                       http://brandonwheelz.in/index.php<br>
                       <p></p>
                       <b></b>";
...
// Handle Internal Payment Request Update
if ($_POST['updateInternal_payment']) {
  
  $internal_id = $_POST["internal_id"];
  $internal_employee_name = $_POST['internal_employee_name_edit'];
  $internal_total_amount = $_POST['internal_total_amount_edit'];
  
  $updateFields = array();
  
  // Handle file upload if new file is provided
  if(isset($_FILES['internal_upload_details_edit']) && $_FILES['internal_upload_details_edit']['error'] == 0) {
    $file_name_details = $_FILES['internal_upload_details_edit']['name'];
    $upload_dir_details = 'Internal_Payment_Details/';
    $file_path_details = $upload_dir_details . $file_name_details;
    move_uploaded_file($_FILES['internal_upload_details_edit']['tmp_name'], $file_path_details);
    $updateFields[] = "upload_details='$file_name_details'";
  }
  
  // Base update query
  $stmt_update = "UPDATE internal_payment_request SET 
                  employee_name='$internal_employee_name', 
                  total_amount='$internal_total_amount'";
  
  // Add file update field if file was uploaded
  if(!empty($updateFields)) {
    $stmt_update .= ", " . implode(", ", $updateFields);
  }
  
  $stmt_update .= " WHERE id='$internal_id'";

  $rslt_update = mysqli_query($conn, $stmt_update);
  
 /* if($rslt_update) {
    echo '<script>alert("Internal Payment Request updated successfully!")</script>';
  }*/
}
?>
...
<head>
  ...
  <style>
    ...
    #adduserButton, #addInternalButton {
      ...
      <section class="content" style="background: linear-gradient(327deg, #ff0000a3, #FFFFFF), url('../images/bwz_erp_bg.jpg'); background-size: cover; background-position: center center;">
        <div class="container-fluid">
          <div class="card-footer">
            ...
            <button type="submit" id="adduserButton" onclick="showadd_user()"><i class="fa fa-user-plus" aria-hidden="true"></i> Add Vendor Payment Request</button>
            <button type="submit" id="addInternalButton" onclick="showadd_internal()"><i class="fa fa-money-bill" aria-hidden="true"></i> Add Internal Payment Request</button>
                        ...
                        <table class="table table-striped projects table-bordered" id="table_camp">
                          <thead>
                            <tr>
                              ...
                              <th style="width:20%">Accounts Status</th>
                              <th style="width:20%">CEO Status</th>
                              <th style="width:20%">Director Status</th>
                              <th style="width:70%">Op's Comments</th>
                        ...
                        <table class="table table-striped projects table-bordered" id="table_internal">
                          ...
                          <tbody>
                            ...
                            while ($row_internal = mysqli_fetch_assoc($rslt_internal)) {
                              $details_file = "Internal_Payment_Details/" . $row_internal["upload_details"];
                            ?>
                              <tr>
                                ...
                                <td>
                                  <button class="btn btn-info btn-sm" onclick="internalEditFun('<?php echo $row_internal["id"]; ?>')">
                                    <i class="fas fa-pencil-alt"></i> Edit
                                  </button>
                                </td>
                              </tr>
                  ...
                  <table>
                    ...
                    <tr>
                        ...
                        <div class="form-group" style="width:250px;">
                          ...
                          <input type="text" class="form-control" pattern="^[0-9\s]*$" title="Please enter only numbers" id="txt_invoice_val" name="txt_invoice_val" placeholder="Enter Invoice Value" required>
                        </div>
                      </th>
                      <th>
                        <div class="form-group" style="width:250px;">
        ...
        <!-- The Modal Add Internal Payment -->
        <div class="modal fade" id="myInternal" style="opacity: 3;top : 104px !important; overflow-y:scroll;">
          <div class="modal-dialog">
            <div class="modal-content" style="top:40px;width: 600px; margin-left: 0px;">
              <!-- Modal Header -->
              <button type="button" class="close34" data-dismiss="modal" style="margin-left: 96%;">&times;</button>
              <div class="modal-header">
                <h4 class="modal-title">Add Internal Payment Request</h4>
              </div>
              <div class="modal-body">
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                  <div class="form-group">
                    <label for="internal_employee_name">Employee Name<span style="color:red;">*</span></label>
                    <select class="form-control select2" name="internal_employee_name" id="internal_employee_name" required>
                      <option value="">Select Employee Name</option>
                      <?php
                      $stmt_emp = "SELECT user_name FROM users WHERE user_level='Operations'";
                      $rslt_emp = mysqli_query($conn, $stmt_emp);
                      while ($row_emp = mysqli_fetch_assoc($rslt_emp)) {
                      ?>
                        <option value="<?php echo $row_emp["user_name"]; ?>"><?php echo $row_emp["user_name"]; ?></option>
                      <?php } ?>
                    </select>
                  </div>
                  
                  <div class="form-group">
                    <label for="internal_upload_details">Upload Details</label>
                    <input type="file" class="form-control" id="internal_upload_details" name="internal_upload_details" accept=".pdf">
                    <span style="font-size: 10px;font-family: Open Sans;"><b>Only PDF</b></span>
                  </div>
                  
                  <div class="form-group">
                    <label for="internal_total_amount">Total Amount<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" pattern="^[0-9.]*$" title="Please enter only numbers" id="internal_total_amount" name="internal_total_amount" placeholder="Enter Total Amount" required>
                  </div>
                  
                  <div class="form-group">
                    <label for="internal_comments">Comments<span style="color:red;">*</span></label>
                    <textarea class="form-control" id="internal_comments" name="internal_comments" placeholder="Enter Comments" rows="3" required></textarea>
                  </div>
                  
                  <div class="card-footer">
                    <input type="submit" class="btn btn-success" name="addInternal_payment" value="Submit">
                  </div>
                </form>
              </div>
        ...
        <!-- The Modal Edit Internal Payment -->
        <div class="modal fade" id="myModal_edit_internal" style="opacity: 3;top : 104px !important; overflow-y:scroll;">
          <div class="modal-dialog">
            <div class="modal-content" style="top:40px;width: 600px; margin-left: 0px;">

              <!-- Modal Header -->
              <div class="modal-header">
                <h4 class="modal-title">Update Internal Payment Request</h4>
                <button type="button" class="close456" data-dismiss="modal">&times;</button>
              </div>
              ...
              <div class="modal-body">
                <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
                  <div class="form-group">
                    <label for="internal_employee_name_edit">Employee Name<span style="color:red;">*</span></label>
                    <select class="form-control select2" name="internal_employee_name_edit" id="internal_employee_name_edit" required>
                      <option value="">Select Employee Name</option>
                      <?php
                      $stmt_emp = "SELECT user_name FROM users WHERE user_level='Operations'";
                      $rslt_emp = mysqli_query($conn, $stmt_emp);
                  ...
                  <div class="form-group">
                    <label for="internal_upload_details_edit">Upload Details</label>
                    <input type="file" class="form-control" id="internal_upload_details_edit" name="internal_upload_details_edit" accept=".pdf">
                    <span style="font-size: 10px;font-family: Open Sans;"><b>Only PDF (Leave empty if no change)</b></span>
                  </div>
                  
                  <div class="form-group">
                    <label for="internal_total_amount_edit">Total Amount<span style="color:red;">*</span></label>
                    <input type="text" class="form-control" pattern="^[0-9.]*$" title="Please enter only numbers" id="internal_total_amount_edit" name="internal_total_amount_edit" placeholder="Enter Total Amount" required>
                  </div>
                  
                  <div class="card-footer">
                    <input type="hidden" value="" id="internal_id" name="internal_id">
                    <input type="submit" class="btn btn-success" name="updateInternal_payment" value="Update">
                  </div>
                </form>
              </div>
...
$('#table_internal').dataTable({
  "order": []
    ...
    function ingroupEditFun(val) {
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          //alert(res[8]);
          document.getElementById('txt_invoice_val').value = res[8];
    ...
    function internalEditFun(val) {
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
          ...
          document.getElementById('internal_employee_name_edit').value = res[0];
          document.getElementById('internal_total_amount_edit').value = res[1];
          document.getElementById('internal_id').value = res[2];
        }
      };
      xhttp.open("GET", "edit_internal_payment_request.php?id=" + val, true);
      xhttp.send();
      
      // Get the modal
      var modal2 = document.getElementById('myModal_edit_internal');
      // Get the <span> element that closes the modal
      var span2 = document.getElementsByClassName("close456")[0];
      // When the user clicks the edit button, open the modal 
    ...
    function showadd_internal() {
      document.getElementById('myInternal').style.display = 'block'
      // Get the modal
      var modal34 = document.getElementById('myInternal');
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#internal_employee_name').select2();
      $('#internal_employee_name_edit').select2();
    });
  </script>
</body>
