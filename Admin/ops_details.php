...
if($_POST['accept_ceo']){
	...
	$id = $_POST["ord_id"];
	$sale_name = $_POST["ord_sale"];

  $stmt_update = "UPDATE order_details SET Director_status='Accepted',status='Completed' WHERE id='$id'";

  $rslt_update = mysqli_query($conn, $stmt_update);

  $stmt_ops = "SELECT email,user_name from users where user_name='$sale_name';";
  $rslt_ops = mysqli_query($conn, $stmt_ops);
  $row_ops = mysqli_fetch_row($rslt_ops);
  $sale_email = $row_ops[0];
...
// Sale Mail
  ...
  $stmt_hod = "SELECT email,user_name from users where user_level='Accounts';";
  ...
  $stmt_op = "SELECT email,user_name from users where user_level='Operations';";
  ...
  $mail->Subject = "New Order Approved";
  // Sender and recipient details
  $mail->setFrom('erp@brandonwheelz.com', 'New Order Approved');

  // Email content
  $mail->isHTML(true);

  $mail->Body = "<p>Dear all, </p> <h3> New Sales order has been approved by CEO kindly check the dash board for more details<br></h3>
...
if($_POST['reject_order']) {
  ...
  $stmt_update = "UPDATE order_details SET Director_status='Rejected',comments='$txt_reason' WHERE id='$txt_id'";
  ...
  $mail->isHTML(true);
  $mail->Subject = "Payment Confirmation For New Order";
  $mail->Body = "<p>Dear $acc_user, </p> <h3>Your payment confirmation for new Order has been rejected by CEO, kindly check the dash board for more details<br></h3>
                  ...
                  <div class="card-body p-0">
                    <table class="table table-striped projects table-bordered" id="table_camp">
                      ...
                      <tbody>
                        ...
                        <?php
                        $stmt_select = "SELECT * from order_details order by created_date desc;";
                        ...
                        //   $Po_file = "../Sales/PO/" . $row["PO"];
                          ...
                          $PI_file = "../Accounts/Customer_invoice/" . $row["customer_invoice"];
