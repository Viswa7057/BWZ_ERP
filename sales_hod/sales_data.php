...
//Total Completed sales
 $stmt_Totaluser="SELECT count(id) as total_count from order_details where HOD_Status='Accepted';";
	                           $rslt_Totaluser= mysqli_query($conn,$stmt_Totaluser);
	                           $row_Totaluser= mysqli_fetch_row($rslt_Totaluser);
							   $Total_sales = $row_Totaluser[0];
							   
//Total New sales
 $stmt_NEW_sale="SELECT count(id) as new_count from order_details where status='NEW';";
...
//Total Rejected sales
 $stmt_Reject_sale="SELECT count(id) as reject_count from order_details where ceo_status='Rejected';";
			 ...
			 <table class="table m-0 tableFixHead" id="completed_sales">
				  ...
				  <table class="table m-0 tableFixHead" id="pending_sales">
					...
					<?php 
						...
						$stmt_select = "SELECT * from order_details where status='NEW'";
				  ...
				  <table class="table m-0 tableFixHead" id="rejected_sales">
					...
					<?php 
						...
						$stmt_select = "SELECT * from order_details where ceo_status='Rejected'";
                        $rslt_rs = mysqli_query($conn, $stmt_select);
