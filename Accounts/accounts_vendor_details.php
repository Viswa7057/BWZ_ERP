...
$targetFile_VD = basename($_FILES["invoice"]["name"]);
  ...
  $stmt_update = "UPDATE vendor_reg SET PO_invoice='$file_name_VD',acc_status='Accepted',status='CEO' WHERE id='$id'";
//echo $stmt_update; exit;
  ...
  // After submit the form Send mail to RM
      ...
      $mail->isHTML(true);
      $mail->Subject = "Vendor PO Approve Request";
      $mail->Body = "<p>Dear $ops_user, </p> <h3>Kindly check the Vendor PO doc for the $company_name & Please approve.<br></h3>
  ...
  // After submit the form Send mail to RM
      ...
      $mail->Body = "<p>Dear $op_name, </p> <h3>Kindly check the attached vendor PO doc for the $company_name.<br></h3>
                     ...
                     <p></p>
                     <b></b>";
	  $filePath = "PO/" .$file_name_VD; // Replace with the path to your file
...
$stmt_update = "UPDATE vendor_reg SET acc_status='Rejected',status='Rejected' WHERE id='$txt_id'";
$rslt_update = mysqli_query($conn, $stmt_update); 
  ...
  $mail->Body = "<p>Dear $op_name, </p> <h3>Your vendor registration has been rejected by BWZ team, because of $txt_reason.<br></h3>
                     <p></p>
                     <b></b>";
                  ...
                  <div class="card-body p-0">
                    <table class="table table-striped projects table-bordered" id="table_camp">
                      ...
                      <tbody>
                        ...
                        <?php
                        $stmt_select = "SELECT * from vendor_reg where graph_status='Accepted' order by id desc;";
                        ...
                        ?>
                          <tr>
							...
							<?php if ($row["PO_invoice"] == "") { ?>
                                <?php } else { ?>
								<button class="btn btn-success" onclick="view_PO_Invoice('<?php echo $PO_file; ?>')"><i class="fa fa-eye"></i>PO</button>
								 <?php
                                }
                                ?>
							</td>
                           ...
                           <td>
                                ...
                                <?php } else { ?>
                                  <button class="btn btn-success" style="width: 91px;" onclick="acceptInstallation('<?php echo $row['id']; ?>','<?php echo $row['vendor_name']; ?>')">Accept</button>
                                  <br>
                                  <br>
...
function view_PO_Invoice(PO_Invoice) {
      document.getElementById('pdfViewer').innerHTML = '<object data="' + PO_Invoice + '" type="application/pdf" width="100%" height="100%"></object>';
      document.getElementById('pdfModal').style.display = 'flex';
    }
