    ...
    <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="height:90px;">
      ...
      <ul class="navbar-nav">
        <li class="nav-item d-none d-sm-inline-block">
          <h2 style="margin-left:50px;color: #b52f34;"><b><i class="fa fa-th-large" aria-hidden="true"></i> Accounts Dashboard</b></h2>
        </li>
      ...
      xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
         ...
         console.log(result1);
		  ...
		  $('#Completed_invoice').html(result[3]);
          $('#pending_invoice').html(result[4]);
		  
		  $('#Completed_po').html(result[5]);
          $('#pending_po').html(result[6]);
		  
		  //DATA
          $('#CompletedSaleData').html(result[7]);
          $('#pendingSaleData').html(result[8]);
          $('#rejectedSaleData').html(result[9]);
		  
		  $('#Completed_Invoice_Data').html(result[10]);
          $('#pending_Invoice_Data').html(result[11]);
		  
		  $('#Completed_PO_Data').html(result[12]);
          $('#pending_PO_Data').html(result[13]);
          ...
          var myChart = new Chart(ctx, {
            type: 'bar',
            data: {
              labels: ['Completed Payments', 'Pending Payments', 'Rejected Payments', 'Completed Invoice', 'Pending Invoice', 'Completed PO', 'Pending PO'],
              datasets: [{
                label: 'Data',
                data: [
