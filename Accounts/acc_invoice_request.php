...
if ($_POST['accept_neworder']) {
  ...
  if (!in_array($fileExtension_VD, $allowedFileTypes_VD)) {
    ...
    $mail->setFrom('erp@brandonwheelz.com', 'Invoice');

    // Email content
    $mail->isHTML(true);

    $mail->Body = "<p>Dear $sale_name, </p> <h3>Kindly check the attached Invoice doc for the $company_name.<br></h3>
                     Click the below link for login to see more details<br>
					 http://brandonwheelz.in/index.php<br>
                     <p></p>
                     <b></b>";
    $filePath = "Customer_invoice/" . $file_name_VD; // Replace with the path to your file
    $mail->addAttachment($filePath);
    // Add TO recipient
    $mail->addAddress("$sale_email", "$sale_name");
    ...
    <nav class="main-header navbar navbar-expand navbar-white navbar-light" style="height:90px;">
      ...
      <ul class="navbar-nav">
        <li class="nav-item d-none d-sm-inline-block">
          <h2 style="margin-left:50px;color: #b52f34;"><b><i class="fa fa-check-square" aria-hidden="true"></i> Invoice Request</b></h2>
        </li>
                      ...
                      $stmt_select = "SELECT * from order_details where ceo_status='Accepted'";
                        ...
                        ?>
                          <tr>
                            ...
                            <td>
                              <?php echo $row["Total_Value_Without_GST"]; ?>
                            </td>
                            <td>
                            ...
                            <td>
                              ...
                              ?>
                                <button class="btn btn-primary" onclick="viewCheck('<?php echo $invoice_file; ?>')"><i class="fa fa-eye"></i> Invoice</button>
                              <?php
                              } else {
                              }
                              ?>
                            ...
                            <td>
                              <button class="btn btn-success" style="width: 91px;" onclick="acceptInstallation('<?php echo $row['id']; ?>','<?php echo $row['company_name']; ?>','<?php echo $row["sales_person"]; ?>','<?php echo $row["email"]; ?>')">Upload Invoice</button>
                              <br>
      ...
      <div class="modal-content" style="top:40px;height:450px;overflow-y:auto;">
        ...
        <div class="modal-body">
          <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" value="" id="sale_name" name="sale_name">
            <input type="hidden" value="" id="customer_email" name="customer_email">
...
<script>
  ...
  <script>
    ...
    function acceptInstallation(Aval_id, Acust_name, sale, cust_email) {
      //alert(cust_name);
      document.getElementById('acceptID').value = Aval_id;
      document.getElementById('txt_vendorname').value = Acust_name;
      document.getElementById('sale_name').value = sale;
      document.getElementById('customer_email').value = cust_email;
