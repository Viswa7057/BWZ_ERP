...
if ($_POST['accept_neworder']) {
  ...
  $stmt_update = "UPDATE vendor_reg SET graph_status='Accepted',status='Accounts' WHERE id='$id'";
...
$stmt_ops = "SELECT email,user_name from users where user_level='Accounts';";
  ...
  // After submit the form Send mail to RM
      ...
      $mail->isHTML(true);
      $mail->Subject = "New Vendor Details Request For PO";
      $mail->Body = "<p>Dear $ops_user, </p> <h3>New Vendor Request has been added and requested for PO. Kindly check the dash board for more details.<br></h3>
...
$stmt_update = "UPDATE vendor_reg SET graph_status='Rejected',status='Rejected' WHERE id='$txt_id'";
$rslt_update = mysqli_query($conn, $stmt_update); 
...
<body class="hold-transition sidebar-mini" id="sampleDiv_zoom">
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
         ...
         <li><a href="graph_dashboard.php" class="menu-link"><i class="fa fa-th-large " aria-hidden="true"></i> Dashboard</a></li><br><br>
		  <li><a href="graph_vendor_details.php" class="menu-link"><i class="fa fa-shopping-cart" aria-hidden="true"></i> New Vendor Info</a></li><br><br>
                  ...
                  <div class="card-body p-0">
                    <table class="table table-striped projects table-bordered" id="table_camp">
                      ...
                      <tbody>
                        ...
                        <?php
                        $stmt_select = "SELECT * from vendor_reg where ops_status='Accepted' order by id desc;";
                        ...
                        ?>
                          <tr>
                            ...
                            <td>
                              <button class="btn btn-primary" onclick="togglePopup('<?php echo $VRF_file; ?>','<?php echo $cheq_file; ?>','<?php echo $aggrement_file; ?>','<?php echo $DC_file; ?>','<?php echo $VD_file; ?>')"><i class="fa fa-eye"></i> DOC</button><br><br>
                           ...
                           <td>
                                <?php if ($row["graph_status"] == "Accepted" || $row["graph_status"] == "Rejected") { ?>
                                <?php } else { ?>
  ...
  <div id="popup" class="popup" style="display: none;margin-top:-450px;margin-left:800px;">
    ...
    <button class="btn btn-success" id="viewAggreButton"><i class="fa fa-eye"></i> Agreement</button><br><br>
	<button class="btn btn-success" id="viewDCButton"><i class="fa fa-eye"></i> DC Report</button>
    <button class="btn btn-success" id="viewVDButton"><i class="fa fa-eye"></i> Vehicle Details</button>
  </div>
  ...
  <script>
    function togglePopup(PO, Check, Aggre, DC, VD) {
      //alert(SOW);
      ...
      if (popup.style.display === "none") {
        ...
        setupViewAggreButton(Aggre);
        setupViewDCButton(DC);
        setupViewVDButton(VD);		// Setup the button to view payment when the popup is shown
      } else {
	...
	function setupViewDCButton(DC) {
      var viewDCButton = document.getElementById("viewDCButton");
      // Ensure we don't attach multiple event listeners to the same button
      viewDCButton.onclick = null; // Remove existing onclick to avoid multiple triggers
      viewDCButton.onclick = function() {
        viewDC(DC);
      };
    }
	
	function setupViewVDButton(VD) {
      var viewVDButton = document.getElementById("viewVDButton");
      // Ensure we don't attach multiple event listeners to the same button
      viewVDButton.onclick = null; // Remove existing onclick to avoid multiple triggers
      viewVDButton.onclick = function() {
        viewVD(VD);
      };
    }
  ...
  <script>
    ...
    function checkLevel() {
      ...
      if (type == "Poc") {
        ...
        document.getElementById("SOW_attachment").readonly = false;
        document.getElementById("MSA_attachment").readonly = false;
      } else {
        document.getElementById("po_no").readonly = true;
        document.getElementById("amount").readonly = true;
        document.getElementById("payment").readonly = true;
        document.getElementById("SOW_attachment").readonly = true;
        document.getElementById("MSA_attachment").readonly = true;
      }
    }
    ...
    function txt_checkLevel() {
      ...
      if (txt_type == "Poc") {
        document.getElementById("txt_po_no").readonly = false;
        document.getElementById("txt_amount").readonly = false;
        document.getElementById("txt_payment").readonly = false;
        document.getElementById("txt_SOW_attachment").readonly = false;
        document.getElementById("txt_MSA_attachment").readonly = false;
      } else {
        document.getElementById("txt_po_no").readonly = true;
        document.getElementById("txt_amount").readonly = true;
        document.getElementById("txt_payment").readonly = true;
        document.getElementById("txt_SOW_attachment").readonly = true;
        document.getElementById("txt_MSA_attachment").readonly = true;
      }
    }
  </script>
</body>
